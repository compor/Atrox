...
ATROX_SKIP_TESTS="OFF"
ATROX_SKIP_TESTS=${GTEST_ROOT:=ON}
...
[[ -z ${ITERATORRECOGNITION_DIR} ]] && echo "ITERATORRECOGNITION_DIR is not set"
...
CMAKE_OPTIONS="${CMAKE_OPTIONS} -DATROX_SKIP_TESTS=${ATROX_SKIP_TESTS}"
CMAKE_OPTIONS="${CMAKE_OPTIONS} -DPedigree_DIR=${PEDIGREE_DIR}"
CMAKE_OPTIONS="${CMAKE_OPTIONS} -DIteratorRecognition_DIR=${ITERATORRECOGNITION_DIR}"
